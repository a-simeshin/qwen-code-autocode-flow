...
```bash
# In pom.xml look for:
grep_search("<java.version>", path="pom.xml")        # e.g., <java.version>17</java.version>
grep_search("<maven.compiler.source>", path="pom.xml")
grep_search("<release>", path="pom.xml")             # e.g., <release>21</release>

# In build.gradle look for:
grep_search("sourceCompatibility", path="build.gradle")
grep_search("languageVersion", path="build.gradle")
...
─────────────────────────────────────────────────────────────────────────────
HAS_JAVA +           ANY Java task                      → .qwen/refs/java-patterns.md (ALWAYS!)
                     controller, service, entity,       → + Context7: spring-boot
                     repository, api, endpoint

HAS_JAVA +           test, тест, junit, jupiter,       → .qwen/refs/java-testing.md
...
HAS_REACT +          react, component, hook, button,    → .qwen/refs/react-patterns.md
...
grep_search("Dashboard|Metric|Controller")
```
...
│   │
│   └─ HAS_JAVA?
│       ├─ grep_search("<java.version>|<release>|sourceCompatibility", pom.xml/build.gradle)
