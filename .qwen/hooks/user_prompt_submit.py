...
def manage_session_data(session_id, prompt, name_agent=False):
    ...
    sessions_dir = get_project_dir() / ".qwen" / "data" / "sessions"
    ...
    if name_agent and "agent_name" not in session_data:
        ...
        try:
            result = subprocess.run(
                ["uv", "run", "--no-project", ".qwen/hooks/utils/llm/ollama.py", "--agent-name"],
                capture_output=True,
                text=True,
            ...
            try:
                result = subprocess.run(
                    ["uv", "run", "--no-project", ".qwen/hooks/utils/llm/anth.py", "--agent-name"],
                    capture_output=True,
                    text=True,
                    timeout=10
                )
