...
READ_ONLY_PATTERNS = {
    "read_file": lambda tool_input: True,  # All read_file operations
    "glob": lambda tool_input: True,  # All glob operations
    "grep_search": lambda tool_input: True,  # All grep_search operations
    "run_shell_command": lambda tool_input: is_safe_bash_command(tool_input.get("command", "")),
}
...
def main():
    try:
        ...
        parser = argparse.ArgumentParser(
            description="PermissionRequest hook for Qwen Code"
        )
        parser.add_argument(
            "--auto-allow",
            action="store_true",
            help="Auto-allow read-only operations (read_file, glob, grep_search, safe run_shell_command)"
        )
        parser.add_argument(
            "--log-only",
            action="store_true",
