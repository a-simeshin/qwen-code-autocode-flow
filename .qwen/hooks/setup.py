...
from utils.paths import get_log_dir, get_project_dir
...
def persist_env_variable(name, value):
    """Persist an environment variable via QWEN_ENV_FILE."""
    env_file = os.environ.get('QWEN_ENV_FILE')
    if env_file:
...
def install_project_dependencies():
    ...
    if (project_dir / 'pyproject.toml').exists() and not (project_dir / 'requirements.txt').exists():
        try:
            # Try uv first
...
def get_project_info(cwd):
    ...
    if (project_dir / '.qwen').exists():
        info.append("Qwen Code configuration directory present")

        # Check for CLAUDE.md or CONTEXT.md
        for context_file in ['CLAUDE.md', 'CONTEXT.md']:
            context_path = project_dir / '.qwen' / context_file
            if context_path.exists():
                info.append(f"Found {context_file} in .qwen/")
